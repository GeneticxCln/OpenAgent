...
class TestWorkQueue:
    ...
    async def test_queue_user_limits(self, work_queue):
        ...
        result = await work_queue.submit(self.simple_task, 5, user_id="test_user")
        assert result == 10

        # Try to exceed concurrent limit
        with pytest.raises(Exception):  # Should raise AgentError
            tasks = [
                work_queue.submit(self.simple_task, i, delay=1.0, user_id="test_user")
    ...
    async def test_queue_timeout(self, work_queue):
        ...
        with pytest.raises(Exception):  # Should raise AgentError for timeout
            await work_queue.submit(self.simple_task, 5, delay=2.0, timeout=1.0)
    ...
    async def test_queue_cancellation(self, work_queue):
        ...
        task = asyncio.create_task(work_queue.submit(self.simple_task, 5, delay=10.0))
...
class TestPerformanceLoad:
    ...
    async def test_cache_load(self):
        ...
        try:
            ...
            tasks = [cache.put(f"model-{i}", models[i]) for i in range(30)]
            await asyncio.gather(*tasks)

            # Retrieve them concurrently
            retrieve_tasks = [cache.get(f"model-{i}") for i in range(30)]
    ...
    async def test_memory_management_load(self):
        ...
        manager = MemoryManager(gc_frequency=0.5, auto_cleanup=True)  # Fast GC
    ...
    if len(sys.argv) > 1:
        ...
        if test_category == "basic":
            pytest.main(
                [
                    "-v",
                    "tests/test_performance.py::TestModelCache",
