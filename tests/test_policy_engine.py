...
from openagent.core.policy import (
    ...
    AuditEntry, get_policy_engine, configure_policy
)
...
class TestPolicyEvaluation:
    ...
    async def test_safe_command_evaluation(self, policy_engine):
        ...
        assert "Command appears safe" in " ".join(reasons) or "allowlisted" in " ".join(reasons).lower()
    ...
    async def test_sudo_command_high_risk(self, policy_engine):
        ...
        decision, risk, reasons = await policy_engine.evaluate_command("sudo apt update")
    ...
    async def test_medium_risk_requires_approval(self, policy_engine):
        ...
        decision, risk, reasons = await policy_engine.evaluate_command("chmod 755 file.txt")
...
class TestRiskAssessment:
    ...
    def test_destructive_operations_high_risk(self, policy_engine):
        ...
        risk, reasons = policy_engine._assess_risk("rm -rf /tmp/data", ["rm", "-rf", "/tmp/data"])
    ...
    def test_system_path_writes_high_risk(self, policy_engine):
        ...
        risk, reasons = policy_engine._assess_risk("echo test > /etc/passwd", ["echo", "test"])
    ...
    def test_permission_changes_medium_risk(self, policy_engine):
        ...
        risk, reasons = policy_engine._assess_risk("chmod 644 file.txt", ["chmod", "644", "file.txt"])
...
class TestAuditLogging:
    ...
    def test_audit_export_json(self, policy_engine):
        ...
        # Create test entry
        asyncio.run(policy_engine.audit_command(
