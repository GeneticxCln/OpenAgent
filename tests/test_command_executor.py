...
def mock_policy_engine():
    ...
    engine.policy = CommandPolicy(
        default_mode="explain_only", audit_enabled=False  # Disable audit for tests
    )
...
class TestCommandExecutorBasics:
    ...
    async def test_explain_only_mode(self, executor):
        ...
        result = await executor.execute(
            {"command": "echo 'test'", "explain_only": True}
        )
    ...
    async def test_command_risk_assessment(self, executor):
        ...
        # Test low risk
        result = await executor.execute({"command": "ls -la", "explain_only": True})
        assert result.metadata.get("risk_level") == "low"

        # Test high risk - rm command
        result = await executor.execute(
            {"command": "rm -rf /tmp/test", "explain_only": True}
        )
        assert result.metadata.get("risk_level") in ["high", "blocked"]

        # Test high risk - sudo
        result = await executor.execute(
            {"command": "sudo apt update", "explain_only": True}
        )
        assert result.metadata.get("risk_level") in ["high", "blocked"]
    ...
    async def test_suggestion_generation(self, executor):
        ...
        with patch.object(
            ...
            with patch("openagent.tools.system.get_policy_engine") as mock_get_policy:
                mock_engine = Mock()
                mock_engine.evaluate_command = AsyncMock(
                    return_value=(PolicyDecision.ALLOW, RiskLevel.LOW, ["safe command"])
                )
                mock_engine.audit_command = AsyncMock()
                mock_get_policy.return_value = mock_engine

                result = await executor.execute(
                    {"command": "foobar", "explain_only": False, "confirm": True}
                )
...
class TestPolicyIntegration:
    ...
    async def test_sandbox_execution(self, unsafe_executor):
        ...
        with patch("openagent.tools.system.get_policy_engine") as mock_get_policy:
            ...
            mock_engine.audit_command = AsyncMock()
            mock_engine.execute_sandboxed = AsyncMock(
                return_value={
                    "success": True,
            ...
            result = await unsafe_executor.execute(
                {"command": "echo 'test'", "explain_only": False, "sandbox": True}
            )
...
class TestCommandExecution:
    ...
    async def test_successful_command(self, unsafe_executor):
        ...
        with patch("openagent.tools.system.get_policy_engine") as mock_get_policy:
            ...
            with patch.object(
                ...
                result = await unsafe_executor.execute(
                    {"command": "echo 'Hello, World!'", "explain_only": False}
                )
    ...
    async def test_failed_command(self, unsafe_executor):
        ...
        with patch("openagent.tools.system.get_policy_engine") as mock_get_policy:
            ...
            with patch.object(
                ...
                result = await unsafe_executor.execute(
                    {"command": "cat /nonexistent/file", "explain_only": False}
                )
    ...
    async def test_command_timeout(self, unsafe_executor):
        ...
        with patch("openagent.tools.system.get_policy_engine") as mock_get_policy:
            ...
            with patch.object(
                unsafe_executor, "_execute_command", side_effect=mock_timeout_command
            ):
                result = await unsafe_executor.execute(
                    {"command": "sleep 60", "explain_only": False}
                )
...
class TestExplanations:
    ...
    async def test_basic_command_explanations(self, executor):
        ...
        for command, keywords in commands_and_keywords:
            result = await executor.execute({"command": command, "explain_only": True})
    ...
    async def test_complex_command_explanation(self, executor):
        ...
        result = await executor.execute(
            {"command": "ls -la --color=auto /tmp", "explain_only": True}
        )

        assert result.success is True
        assert (
            "-la" in result.content
            or "-l" in result.content
            or "flags" in result.content
        )
...
class TestErrorHandling:
    ...
    async def test_exception_handling(self, executor):
        ...
        with patch("openagent.tools.system.get_policy_engine") as mock_get_policy:
            ...
            result = await executor.execute(
                {"command": "echo test", "explain_only": False}
            )
...
async def test_audit_logging():
    ...
    from openagent.core.policy import CommandPolicy, PolicyEngine
