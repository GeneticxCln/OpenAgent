...
from openagent.core.tool_selector import SmartToolSelector, ToolPlan, ToolCall, ToolIntent
...
class TestToolPlanning:
    ...
    async def test_execute_tool_plan(self, mock_llm, tool_set):
        ...
        # Execute the plan
        with patch.object(tool_set["system_info"], 'execute', return_value=ToolResult(
            success=True,
            content="System: Linux",
    ...
    async def test_handle_tool_execution_failure(self, mock_llm, tool_set):
        ...
        with patch.object(tool_set["command_executor"], 'execute', return_value=ToolResult(
            success=False,
            content="",
...
class TestWorkflowManagement:
    ...
    def test_list_workflows(self, workflow_manager):
        ...
        for wf in existing:
            if wf.name in ["workflow1", "workflow2", "test_workflow", "complex_workflow"]:
...
class TestAgentIntegration:
    ...
    async def test_agent_with_tool_execution(self, mock_llm):
        ...
        with patch.object(agent, '_should_use_tools', return_value=True):
            with patch.object(agent, '_plan_tool_calls', return_value=[
                {"name": "command_executor", "args": {"command": "echo test", "explain_only": True}}
            ]):
                with patch.object(agent, '_execute_tools', return_value={
                    ...
                    mock_llm.generate_response.return_value = "The echo command displays text."
...
class TestEndToEndScenarios:
    ...
    async def test_complete_command_execution_flow(self):
        ...
        from openagent.core.policy import PolicyEngine, CommandPolicy, configure_policy
    ...
    async def test_workflow_execution(self, mock_llm):
        ...
        # Mock tool execution
        with patch.object(system_tool, 'execute', return_value=ToolResult(
            success=True,
