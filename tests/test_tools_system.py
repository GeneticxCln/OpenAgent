...
async def test_command_executor_rejects_dangerous():
    ...
    assert (
        "denied" in (result.error or "").lower()
        or "policy" in (result.error or "").lower()
    )
...
async def test_file_manager_write_and_read(tmp_path):
    ...
    write_res = await fm.execute(
        {"operation": "write", "path": str(file_path), "content": "hello"}
    )
    ...
    async def test_command_timeout(self):
        ...
        with patch.object(tool, "_execute_command") as mock_execute:
            ...
            result = await tool.execute(
                {"command": "sleep 60", "explain_only": False, "confirm": True}
            )
            assert result.success is False
...
class TestFileManagerAdvanced:
    ...
    async def test_list_directory(self, tmp_path):
        ...
        result = await fm.execute({"operation": "list", "path": str(tmp_path)})
    ...
    async def test_file_info_retrieval(self, tmp_path):
        ...
        result = await fm.execute({"operation": "info", "path": str(test_file)})
    ...
    async def test_binary_file_handling(self, tmp_path):
        ...
        with patch("pathlib.Path.read_text") as mock_read:
            ...
            result = await fm.execute({"operation": "read", "path": str(binary_file)})
...
class TestSystemInfo:
    ...
    async def test_system_overview(self):
        ...
        assert result.success is True
        assert (
            "system" in result.content.lower() or "operating" in result.content.lower()
        )
    ...
    async def test_process_info_error_handling(self, mock_process_iter):
        ...
        type(mock_proc).info = PropertyMock(
            side_effect=[first_info, psutil_mod.AccessDenied()]
        )
