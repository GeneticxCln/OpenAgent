...
class MockTool(BaseTool):
    ...
    async def execute(self, input_data):
        ...
        if self.should_fail:
            return ToolResult(success=False, content="", error="Mock tool failure")
        return ToolResult(
            success=True,
...
class TestAgent:
    ...
    async def test_tool_execution_in_message_processing(self):
        ...
        with patch("openagent.core.agent.get_llm") as mock_get_llm:
            ...
            with (
                patch.object(agent, "_should_use_tools", return_value=True),
    ...
    def test_tool_is_relevant_basic_matching(self):
        ...
        assert not agent._tool_is_relevant.__wrapped__(
            agent, calc_tool, "send an email"
        )

        # Test search tool relevance
        search_tool = MockTool("search")
        assert agent._tool_is_relevant.__wrapped__(
            agent, search_tool, "search for information"
        )
        assert agent._tool_is_relevant.__wrapped__(agent, search_tool, "find something")
        assert not agent._tool_is_relevant.__wrapped__(
            agent, search_tool, "calculate numbers"
        )
