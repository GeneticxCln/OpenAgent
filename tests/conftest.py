...
from openagent.core.policy import PolicyAction, PolicyDecision, PolicyEngine
...
def mock_policy_engine():
    ...
    engine = Mock()
    engine.validate_command = AsyncMock(
...
def mock_agent(mock_llm, mock_tools, mock_policy_engine):
    ...
    agent.process_message = AsyncMock(
        return_value=BaseMessage(
            content="Mock agent response", role="assistant", metadata={"success": True}
        )
    )
...
def sample_commands():
    ...
    return {
        "safe": ["echo 'hello world'", "ls -la", "pwd", "date", "whoami"],
        "medium": [
            "chmod 755 file.txt",
...
def sample_file_operations():
    ...
    return {
        "read": {"operation": "read", "path": "/tmp/test.txt", "expected_safe": True},
        "write": {
            "operation": "write",
            "path": "/tmp/output.txt",
...
class TestHelpers:
    ...
    def create_mock_response(
        content: str, success: bool = True, **metadata
    ) -> BaseMessage:
...
def mock_external_deps(monkeypatch):
    ...
    if not os.getenv("OPENAGENT_TEST_REAL_SUBPROCESS"):
        ...
        mock_subprocess.run = Mock(
            return_value=Mock(returncode=0, stdout="Mock output", stderr="")
        )
