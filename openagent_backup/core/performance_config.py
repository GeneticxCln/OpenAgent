...
class ModelCacheConfig(BaseModel):
    ...
    default_ttl: int = Field(
        1800, description="Default cache TTL in seconds (30 minutes)"
    )
    memory_threshold: float = Field(
        0.8, description="Memory threshold for cache eviction"
    )
    preload_models: list = Field(
        default_factory=lambda: ["tiny-llama"],
        description="Models to preload on startup",
    )
    background_warming: bool = Field(
        True, description="Enable background model warming"
    )
...
class WorkQueueConfig(BaseModel):
    ...
    default_timeout: float = Field(
        60.0, description="Default request timeout in seconds"
    )
    ...
    user_max_processing_time: float = Field(
        300.0, description="Max processing time per user"
    )
...
class MemoryManagementConfig(BaseModel):
    ...
    gc_frequency: float = Field(
        300.0, description="Garbage collection frequency in seconds"
    )
    enable_gpu_monitoring: bool = Field(
        True, description="Enable GPU memory monitoring"
    )
    auto_cleanup: bool = Field(True, description="Enable automatic cleanup")

    # Memory thresholds
    system_warning: float = Field(
        80.0, description="System memory warning threshold (%)"
    )
    system_critical: float = Field(
        90.0, description="System memory critical threshold (%)"
    )
    process_warning: int = Field(
        2 * 1024 * 1024 * 1024, description="Process memory warning (bytes)"
    )
    process_critical: int = Field(
        4 * 1024 * 1024 * 1024, description="Process memory critical (bytes)"
    )
...
class ResourceMonitoringConfig(BaseModel):
    ...
    monitoring_interval: float = Field(
        5.0, description="Monitoring interval in seconds"
    )
    ...
    gpu_memory_warning: float = Field(
        75.0, description="GPU memory warning threshold (%)"
    )
    gpu_memory_critical: float = Field(
        90.0, description="GPU memory critical threshold (%)"
    )
    gpu_util_warning: float = Field(
        80.0, description="GPU utilization warning threshold (%)"
    )
    gpu_util_critical: float = Field(
        95.0, description="GPU utilization critical threshold (%)"
    )
    ...
    temperature_warning: float = Field(
        80.0, description="Temperature warning threshold (°C)"
    )
    temperature_critical: float = Field(
        85.0, description="Temperature critical threshold (°C)"
    )
...
class PerformanceConfigurationManager:
    ...
    def update_from_env(self) -> None:
        ...
        if os.getenv("OPENAGENT_MODEL_CACHE_ENABLED"):
            self.config.performance.model_cache.enabled = (
                os.getenv("OPENAGENT_MODEL_CACHE_ENABLED").lower() == "true"
            )

        if os.getenv("OPENAGENT_MODEL_CACHE_SIZE"):
            self.config.performance.model_cache.max_size = int(
                os.getenv("OPENAGENT_MODEL_CACHE_SIZE")
            )

        if os.getenv("OPENAGENT_MODEL_CACHE_TTL"):
            self.config.performance.model_cache.default_ttl = int(
                os.getenv("OPENAGENT_MODEL_CACHE_TTL")
            )

        # Work queue settings
        if os.getenv("OPENAGENT_WORK_QUEUE_ENABLED"):
            self.config.performance.work_queue.enabled = (
                os.getenv("OPENAGENT_WORK_QUEUE_ENABLED").lower() == "true"
            )

        if os.getenv("OPENAGENT_WORK_QUEUE_WORKERS"):
            self.config.performance.work_queue.max_workers = int(
                os.getenv("OPENAGENT_WORK_QUEUE_WORKERS")
            )

        if os.getenv("OPENAGENT_WORK_QUEUE_SIZE"):
            self.config.performance.work_queue.max_queue_size = int(
                os.getenv("OPENAGENT_WORK_QUEUE_SIZE")
            )

        # Memory management settings
        if os.getenv("OPENAGENT_MEMORY_MANAGEMENT_ENABLED"):
            self.config.performance.memory_management.enabled = (
                os.getenv("OPENAGENT_MEMORY_MANAGEMENT_ENABLED").lower() == "true"
            )

        if os.getenv("OPENAGENT_MEMORY_GC_FREQUENCY"):
            self.config.performance.memory_management.gc_frequency = float(
                os.getenv("OPENAGENT_MEMORY_GC_FREQUENCY")
            )

        if os.getenv("OPENAGENT_MEMORY_GPU_MONITORING"):
            self.config.performance.memory_management.enable_gpu_monitoring = (
                os.getenv("OPENAGENT_MEMORY_GPU_MONITORING").lower() == "true"
            )

        # Resource monitoring settings
        if os.getenv("OPENAGENT_RESOURCE_MONITORING_ENABLED"):
            self.config.performance.resource_monitoring.enabled = (
                os.getenv("OPENAGENT_RESOURCE_MONITORING_ENABLED").lower() == "true"
            )

        if os.getenv("OPENAGENT_RESOURCE_MONITORING_INTERVAL"):
            self.config.performance.resource_monitoring.monitoring_interval = float(
                os.getenv("OPENAGENT_RESOURCE_MONITORING_INTERVAL")
            )

        if os.getenv("OPENAGENT_RESOURCE_GPU_MONITORING"):
            self.config.performance.resource_monitoring.enable_gpu_monitoring = (
                os.getenv("OPENAGENT_RESOURCE_GPU_MONITORING").lower() == "true"
            )
...
def initialize_performance_config(
    config: Optional[ExtendedConfig] = None,
) -> PerformanceConfigurationManager:
