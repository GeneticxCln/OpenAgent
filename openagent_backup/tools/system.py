...
class CommandExecutor(BaseTool):
    ...
    def __init__(self, default_explain_only: bool = True, **kwargs):
        # ensure 'terminal' capability by default
    ...
    async def _execute_command(self, command: str) -> Dict[str, Any]:
        ...
        try:
            # Attempt structured exec for a safe subset of commands
            argv = None
            try:
            ...
            if argv:
                try:
                    ...
                    return {
                        "success": success,
                        "output": output,
                        "error": None if success else err.strip(),
                        "exit_code": exit_code,
                        "execution_time": time.time() - start_time,
                    }
                except Exception as e:
                ...
                if strict:
                    return {
                        "success": False,
                        "output": "",
                        "error": "Shell fallback disabled by strict mode",
                        "exit_code": -1,
                        "execution_time": time.time() - start_time,
                    }
                # Fallback: use shell for complex commands (pipes, redirects, etc.)
    ...
    def _build_safe_argv(self, command: str) -> Optional[List[str]]:
        """Build argv for a safe subset of commands; return None if unsafe/unsupported.
        Safe commands: ls, pwd, cat, echo, which, env, id, whoami, date, grep, find
        Disallow any shell metacharacters or redirections.
        """
        # Disallow metacharacters to avoid injections
        forbidden = ["|", ";", "&", ">", "<", "`", "$", "(", ")"]
        ...
        if cmd == "grep":
            ...
            for p in parts[1:]:
                if "-exec" in p or "-delete" in p:
                    return None
        elif cmd in {"ps"}:
            if not allow_flags(flags, ["-e", "-A", "-o", "--sort"]):
                return None
        elif cmd in {"df"}:
            if not allow_flags(flags, ["-h", "-T", "-a"]):
                return None
        elif cmd in {"du"}:
            if not allow_flags(flags, ["-h", "-a", "-s"]):
                return None
        elif cmd in {"ls"}:
            if not allow_flags(flags, ["-l", "-a", "-h", "-1", "-t", "-r"]):
                return None
        # For other whitelisted commands, generic flags are acceptable
...
class FileManager(BaseTool):
    ...
    async def _copy_item(self, src: Path, dst: Path) -> Dict[str, Any]:
        try:
            if src.is_dir():
                if dst.exists():
    ...
    async def _move_item(self, src: Path, dst: Path) -> Dict[str, Any]:
        try:
            dst.parent.mkdir(parents=True, exist_ok=True)
    ...
    async def _delete_item(self, target: Path) -> Dict[str, Any]:
        try:
            if target.is_dir():
    ...
    async def _enforce_file_policy(
        self, operation: str, path: Path, destination: str, options: Dict[str, Any]
    ) -> Optional[Dict[str, Any]]:
        """Check PolicyEngine for file ops: safe paths and approval.
        ...
        def _in_safe(p: Path) -> bool:
            try:
                ...
                rp = p
            for safe in policy_engine.policy.safe_paths:
                try:
                    if str(rp).startswith(str(Path(safe).expanduser().resolve())):
                        return True
                except Exception:
                    continue
            return False

        # Ensure source is in safe paths (for write/delete, we check parent)
        src_check = path if operation != "write" else path.parent
        if not _in_safe(src_check):
            return {
                "success": False,
                "content": "",
                "error": f"Path not in safe paths: {src_check}",
            }

        # Reject operations on restricted paths (retain message consistency)
        for restricted in policy_engine.policy.restricted_paths:
            if str(path).startswith(str(Path(restricted).expanduser())):
                return {
                    "success": False,
                    "content": "",
                    "error": f"Path not in safe paths: {path}",
                }

        # Require safe dest for copy/move
        if operation in {"copy", "move"} and destination:
            dest_path = Path(destination).expanduser()
            if not _in_safe(dest_path):
                return {
                    "success": False,
                    "content": "",
                    "error": f"Destination not in safe paths: {dest_path}",
                }

        # Approval logic: allow low-risk writes within safe paths without approval
        if operation == "write":
            return None

        confirm = bool(options.get("confirm", False))
        # Build a pseudo-command string for policy evaluation context
        pseudo_cmd = f"file:{operation} {path} {destination or ''}".strip()
        decision, risk, reasons = await policy_engine.evaluate_command(pseudo_cmd)
        if decision.name in {"DENY"}:
            return {
                "success": False,
                "content": "",
                "error": f"Operation denied by policy: {', '.join(reasons)}",
            }
        if decision.name in {"REQUIRE_APPROVAL"} and not confirm:
            return {
                "success": False,
                "content": "",
                "error": f"Operation requires approval (risk={risk.value}). Add 'confirm': true to proceed.",
