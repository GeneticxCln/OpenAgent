...
class PluginRegistry:
    ...
    async def get_plugin(self, plugin_name: str) -> Optional[PluginMetadata]:
        ...
        try:
            async with aiosqlite.connect(self.storage_path) as db:
                cursor = await db.execute(
                    "SELECT metadata_json FROM plugins WHERE name = ?", (plugin_name,)
                )
            ...
            async with aiosqlite.connect(self.storage_path) as db:
                ...
                if keywords:
                    ...
                    for name in plugin_names:
                        ...
                        if metadata and any(
                            keyword in metadata.keywords for keyword in keywords
                        ):
    ...
    async def get_statistics(self) -> Dict[str, Any]:
        ...
        try:
            async with aiosqlite.connect(self.storage_path) as db:
                ...
                cursor = await db.execute(
                    "SELECT COUNT(*) FROM plugins WHERE created_at e= ?",
                    (seven_days_ago,),
                )
    ...
    async def export_registry(self, export_path: Path) -> bool:
        ...
        try:
            ...
            async with aiosqlite.connect(self.storage_path) as db:
                cursor = await db.execute(
                    "SELECT metadata_json FROM plugins ORDER BY name"
                )
                rows = await cursor.fetchall()
                ...
                for row in rows:
                    try:
                        ...
                        logger.error(
                            f"Error parsing plugin metadata during export: {e}"
                        )
                        continue
        ...
        try:
            ...
            for plugin_dict in plugins:
                try:
                    ...
                    logger.error(
                        f"Error importing plugin {plugin_dict.get('name', 'unknown')}: {e}"
                    )
                    continue
    ...
    async def _init_database(self) -> None:
        ...
        async with aiosqlite.connect(self.storage_path) as db:
            ...
            await db.execute(
                "CREATE INDEX IF NOT EXISTS idx_plugin_type ON plugins(plugin_type)"
            )
            await db.execute("CREATE INDEX IF NOT EXISTS idx_author ON plugins(author)")
            await db.execute(
                "CREATE INDEX IF NOT EXISTS idx_created_at ON plugins(created_at)"
            )
