...
from .blocks import CommandBlock, BlockManager, BlockRenderer, BlockStatus, BlockType
...
class TerminalRenderer:
    ...
    def render_command_execution(self, command: str, working_dir: Optional[str] = None) -> CommandBlock:
    ...
    def update_block_output(self, block: CommandBlock, output: str, is_error: bool = False):
        ...
        if output and len(output.splitlines()) > 10:
            ...
            sections = self.output_folder.add_content(
                output, 
                "Command Output" if not is_error else "Error Output"
            )
    ...
    def complete_block_execution(self, block: CommandBlock, exit_code: int = 0, duration: Optional[float] = None):
    ...
    def add_ai_response(self, response: str, metadata: Optional[Dict] = None) -> CommandBlock:
    ...
    def _build_normal_display(self) -> Group:
        ...
        recent_blocks = self.block_manager.blocks[-5:] if self.block_manager.blocks else []
    ...
    def update_progress(self, task_name: str, advance: int = 1, description: Optional[str] = None):
    ...
    def render_diff(self, old_content: str, new_content: str, title: str = "Diff") -> Panel:
...
def create_interactive_renderer(console: Optional[Console] = None) -> InteractiveRenderer:
