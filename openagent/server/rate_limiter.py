...
class RateLimiter:
    ...
    def _check_limit(self, identifier: str, limit: int, window: int, storage: Dict) -> Tuple[bool, int]:
    ...
    def check_user_rate_limit(self, user_id: str) -> Tuple[bool, str, int]:
        ...
        if not allowed:
            return False, f"Rate limit exceeded: {self.config.requests_per_minute} requests per minute", retry_after
        ...
        if not allowed:
            return False, f"Rate limit exceeded: {self.config.requests_per_hour} requests per hour", retry_after
        ...
        if not allowed:
            return False, f"Rate limit exceeded: {self.config.requests_per_day} requests per day", retry_after
    ...
    def check_ip_rate_limit(self, ip_address: str) -> Tuple[bool, str, int]:
        ...
        if not allowed:
            return False, f"IP rate limit exceeded: {ip_minute_limit} requests per minute", retry_after
        ...
        if not allowed:
            return False, f"IP rate limit exceeded: {ip_hour_limit} requests per hour", retry_after
        ...
        if not allowed:
            return False, f"IP rate limit exceeded: {ip_day_limit} requests per day", retry_after
    ...
    async def check_request(self, request: Request, user_id: Optional[str] = None) -> None:
        ...
        if user_id:
            user_allowed, user_message, user_retry_after = self.check_user_rate_limit(user_id)
            if not user_allowed:
                raise RateLimitExceeded(user_message, user_retry_after)
    
    def get_rate_limit_info(self, user_id: Optional[str] = None, ip_address: Optional[str] = None) -> Dict[str, any]:
        ...
        if user_id and self.config.enabled:
            ...
            info["user"] = {
                "user_id": user_id,
                "remaining_minute": self._get_remaining_requests(minute_limiter, self.config.requests_per_minute),
                "remaining_hour": self._get_remaining_requests(hour_limiter, self.config.requests_per_hour),
                "remaining_day": self._get_remaining_requests(day_limiter, self.config.requests_per_day)
            }
        ...
        if ip_address and self.config.enabled:
            ...
            info["ip"] = {
                ...
                "remaining_minute": self._get_remaining_requests(minute_limiter, ip_minute_limit),
                "remaining_hour": self._get_remaining_requests(hour_limiter, ip_hour_limit),
                "remaining_day": self._get_remaining_requests(day_limiter, ip_day_limit)
            }
    ...
    def _get_remaining_requests(self, limiter, limit: int) -> int:
        ...
        if isinstance(limiter, SlidingWindowCounter):
            ...
            while limiter.requests and limiter.requests[0] <= current_time - limiter.window_size:
