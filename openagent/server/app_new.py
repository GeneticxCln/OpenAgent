...
from .models import (
    ...
    AnalysisRequest, AnalysisResponse, SystemInfoResponse,
    AgentStatus, AgentCreateRequest, ModelInfo,
    LoginRequest, LoginResponse, User, ErrorResponse,
    HealthResponse
)
from .auth import AuthManager, AuthConfig
from .rate_limiter import RateLimiter, RateLimitConfig, rate_limit_dependency
...
async def check_rate_limit(request: Request, user: User = Depends(get_current_user)) -> None:
...
async def register(request: LoginRequest, current_user: User = Depends(get_current_user)) -> User:
...
@app.post("/generate-code", response_model=CodeResponse, dependencies=[Depends(check_rate_limit)])
...
@app.post("/analyze-code", response_model=AnalysisResponse, dependencies=[Depends(check_rate_limit)])
...
@app.get("/system-info", response_model=SystemInfoResponse, dependencies=[Depends(check_rate_limit)])
...
if __name__ == "__main__":
    ...
    print(f"Authentication: {'Enabled' if auth_manager.config.auth_enabled else 'Disabled'}")
