...
class BaseRequest(BaseModel):
    ...
    agent: Optional[str] = Field(
        None,
        description="Name of the agent to use (defaults to 'default')"
    )
...
class CodeResponse(BaseResponse):
    ...
    suggestions: List[str] = Field(
        default_factory=list,
        description="Additional suggestions or improvements"
    )
...
class AgentStatus(BaseModel):
    ...
    is_processing: bool = Field(..., description="Whether the agent is currently processing")
    tools: List[str] = Field(..., description="List of available tools")
    message_count: int = Field(..., description="Number of messages in conversation history")
    uptime: float = Field(
        default=0.0,
...
class ModelInfo(BaseModel):
    ...
    memory_required: Optional[str] = Field(None, description="Estimated memory requirement")
...
class CommandRequest(BaseRequest):
    ...
    explain_only: bool = Field(
        True,
        description="Only explain the command without executing"
    )
...
class CommandResponse(BaseResponse):
    ...
    explained: bool = Field(
        False,
        description="Whether the command was only explained"
    )
