...
class PluginValidator:
    ...
    async def validate_metadata(self, metadata: PluginMetadata) -> ValidationResult:
        ...
        if not self._check_version_compatibility(metadata.openagent_version):
            errors.append(f"Incompatible OpenAgent version requirement: {metadata.openagent_version}")
        
        if not self._check_version_compatibility(metadata.python_version, self.python_version):
            errors.append(f"Incompatible Python version requirement: {metadata.python_version}")
        ...
        if metadata.permissions:
            ...
            if invalid_perms:
                warnings.extend([f"Unknown permission: {perm}" for perm in invalid_perms])
    ...
    async def validate_plugin_class(self, plugin_class: Type[PluginBase]) -> ValidationResult:
    ...
    def _check_version_compatibility(self, requirement: str, current: Optional[str] = None) -> bool:
    ...
    def _validate_permissions(self, permissions: List[str]) -> List[str]:
        ...
        valid_permissions = {
            ...
            'system_info', 'process_management', 'registry_access',
            'plugin_management', 'user_data', 'sensitive_data'
        }
...
class PluginMarketplace:
    ...
    async def download_plugin(self, plugin_name: str, version: Optional[str] = None) -> bool:
