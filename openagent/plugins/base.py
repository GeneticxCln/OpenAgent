...
class PluginBase(abc.ABC):
    ...
    # Internal methods
    def _set_status(self, status: PluginStatus, error: Optional[Exception] = None) -> None:
        """Set plugin status."""
        self._status = status
        self._error = error
...
def create_plugin(plugin_class: Type[PluginBase], config: Optional[Dict[str, Any]] = None) -> PluginBase:
    """Create a plugin instance with proper validation."""
    if not issubclass(plugin_class, PluginBase):
        raise TypeError(f"Plugin class must inherit from PluginBase, got {plugin_class}")
...
def requires_permissions(*permissions: str):
    ...
    def decorator(cls):
        if not hasattr(cls, '_plugin_metadata'):
            raise AttributeError("Plugin must have metadata before specifying permissions")
...
def plugin_config_schema(schema: Dict[str, Any]):
    ...
    def decorator(cls):
        if not hasattr(cls, '_plugin_metadata'):
            raise AttributeError("Plugin must have metadata before specifying config schema")
