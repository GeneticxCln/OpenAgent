...
from .base import (
    PluginBase, PluginType, PluginStatus, PluginMetadata,
    ToolPlugin, AgentPlugin, IntegrationPlugin
)
                ...
                try:
                    ...
                    if metadata:
                        ...
                        if validation_result.is_valid:
                            ...
                            logger.warning(f"Plugin validation failed for {plugin_file}: {validation_result.errors}")
    ...
    async def load_plugin(self, plugin_name: str, config: Optional[Dict[str, Any]] = None) -> bool:
        ...
        try:
            ...
            if not validation_result.is_valid:
                logger.error(f"Plugin class validation failed for {plugin_name}: {validation_result.errors}")
                return False
            ...
            if not await plugin_instance.initialize():
                ...
                plugin_instance._set_status(PluginStatus.ERROR, Exception("Initialization failed"))
                return False
