...
class PolicyEngine:
    ...
    def __init__(
        self, policy: Optional[CommandPolicy] = None, audit_path: Optional[Path] = None
    ):
        """
    ...
    def _compile_patterns(self):
        ...
        self._allowlist_re = [
            re.compile(p, re.IGNORECASE) for p in self.policy.allowlist_patterns
        ]
        self._denylist_re = [
            re.compile(p, re.IGNORECASE) for p in self.policy.denylist_patterns
        ]
    ...
    def _assess_risk(
        self, command: str, argv: List[str], context: Optional[Dict[str, Any]] = None
        ...
        file_operations = {
            ...
            "file:copy": (3, "file copy operation"),
            "file:delete": (6, "file deletion operation"),
            "file:write": (4, "file write operation"),
        }
                ...
                if full_file_cmd in file_operations:
                    ...
                    if len(argv) > 1:
                        ...
                        if not in_restricted_path:
                            in_safe_path = False
                            for path in self.policy.safe_paths:
                                if file_path.startswith(path):
                                    in_safe_path = True
    ...
    async def audit_command(
        ...
        entry = AuditEntry(
            timestamp=time.time(),
            block_id=block_id or __import__("uuid").uuid4().hex[:12],
            user_id=user_id,
            command=command,
            argv=argv,
    ...
    async def execute_sandboxed(
        ...
        if cwd:
            ...
            if not any(
                str(cwd_path).startswith(str(Path(safe).resolve()))
    ...
    def verify_audit_integrity_report(
    ...
    def export_audit_report(
        ...
        if output_format == "json":
            ...
            summary = {
                ...
                "blocked": sum(
                    1 for e in entries if e.get("policy_decision") == "deny"
                ),
                "risk_levels": {},
                "top_commands": {},
            }
            ...
            for entry in entries:
                ...
                if entry.get("argv"):
                    ...
                    summary["top_commands"][cmd] = (
                        summary["top_commands"].get(cmd, 0) + 1
                    )

            # Sort top commands
            summary["top_commands"] = dict(
                sorted(
                    summary["top_commands"].items(), key=lambda x: x[1], reverse=True
                )[:10]
            )
