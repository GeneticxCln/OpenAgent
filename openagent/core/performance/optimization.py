...
class ModelCache:
    ...
    async def get_model(self, model_name: str, load_config: Optional[Dict[str, Any]] = None) -> Tuple[Any, Any]:
        ...
        self.cache_misses += 1
        loading_task = asyncio.create_task(self._load_model_async(model_name, load_config or {}))
    ...
    async def preload_model(self, model_name: str, load_config: Optional[Dict[str, Any]] = None, priority: int = 1):
        """Preload a model in the background."""
        cache_key = self._get_cache_key(model_name, load_config or {})
        
        with self.cache_lock:
            if cache_key not in self.model_cache and cache_key not in self.loading_tasks:
    ...
    async def _load_model_async(self, model_name: str, load_config: Dict[str, Any]) -> Tuple[Any, Any]:
...
async def optimize_openagent_performance():
    ...
    return {
        "startup_optimization": results[0] if not isinstance(results[0], Exception) else str(results[0]),
        "memory_optimization": "completed" if not isinstance(results[1], Exception) else str(results[1]),
