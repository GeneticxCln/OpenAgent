...
class MemoryOptimizer:
    ...
    def __init__(
        ...
        logger.info(f"MemoryOptimizer initialized: gpu_monitoring={enable_gpu_monitoring}")
    ...
    async def get_memory_report(self) -> Dict[str, Any]:
        ...
        if self._memory_history:
            ...
            history_summary = {
                ...
                "avg_system_percent": sum(s.system_percent for _, s in recent_history) / len(recent_history),
                "avg_process_mb": sum(s.process_memory_mb for _, s in recent_history) / len(recent_history),
                "trend": "increasing" if recent_history[-1][1].process_memory > recent_history[0][1].process_memory else "stable/decreasing"
            }
        ...
        while not self._shutdown_event.is_set():
            try:
                ...
                if stats.system_percent > self.thresholds.system_critical:
                    logger.warning(f"Critical system memory usage: {stats.system_percent:.1f}%")
                elif stats.system_percent > self.thresholds.system_warning:
                    logger.info(f"High system memory usage: {stats.system_percent:.1f}%")
                
                if stats.process_memory > self.thresholds.process_critical:
                    logger.warning(f"Critical process memory usage: {stats.process_memory_mb:.1f}MB")
                elif stats.process_memory > self.thresholds.process_warning:
                    logger.info(f"High process memory usage: {stats.process_memory_mb:.1f}MB")
