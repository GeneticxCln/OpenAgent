...
class ResourceMetrics:
    ...
    def has_resource_pressure(self) -> bool:
        ...
        return (
            self.cpu.is_high_usage or 
            self.memory.is_high_usage or 
            any(gpu.is_high_memory_usage or gpu.is_high_utilization for gpu in self.gpus) or
            self.disk.is_high_usage
        )
...
class ResourceMonitor:
    ...
    def __init__(
        ...
        logger.info(f"ResourceMonitor initialized: interval={monitoring_interval}s, gpu_monitoring={enable_gpu_monitoring}")
    ...
    def get_scaling_recommendations(self, duration: float = 3600) -> List[ScalingRecommendation]:
        """
        ...
        cutoff_time = time.time() - duration
        return [
            rec for rec in self._scaling_recommendations
    ...
    def add_scaling_callback(self, callback: Callable[[ScalingRecommendation], None]) -> None:
        """Add callback for scaling recommendations."""
        self._scaling_callbacks.append(callback)

    async def analyze_performance_trends(self, duration: float = 3600) -> Dict[str, Any]:
        """
        ...
        if history[0].gpus:
            for i in range(len(history[0].gpus)):
                ...
                gpu_memory = [m.gpus[i].memory_percent for m in history if i < len(m.gpus)]
    ...
    def _get_network_stats(self) -> NetworkStats:
        ...
        if self._prev_network_stats and self._prev_timestamp:
            ...
            if time_delta > 0:
                stats.bytes_sent_rate = (net_io.bytes_sent - self._prev_network_stats['bytes_sent']) / time_delta
                stats.bytes_recv_rate = (net_io.bytes_recv - self._prev_network_stats['bytes_recv']) / time_delta
    ...
    def _get_disk_stats(self) -> DiskStats:
        ...
        if disk_io:
            ...
            if self._prev_disk_stats and self._prev_timestamp:
                time_delta = current_time - self._prev_timestamp
                if time_delta > 0:
                    stats.read_rate = (disk_io.read_bytes - self._prev_disk_stats['read_bytes']) / time_delta
                    stats.write_rate = (disk_io.write_bytes - self._prev_disk_stats['write_bytes']) / time_delta
    ...
    async def _generate_scaling_recommendations(self) -> None:
        ...
        if recent_metrics[0].gpus:
            for i in range(len(recent_metrics[0].gpus)):
                gpu_memory_usage = [m.gpus[i].memory_percent for m in recent_metrics if i < len(m.gpus)]
                gpu_util_usage = [m.gpus[i].gpu_percent for m in recent_metrics if i < len(m.gpus)]
                ...
                if gpu_memory_usage and gpu_util_usage:
                    ...
                    if gpu_mem_avg > 80 or gpu_util_avg > 80:
                        recommendations.append(ScalingRecommendation(
                            ...
                            confidence=0.7,
                            parameters={"gpu_id": i, "memory_usage": gpu_mem_avg, "utilization": gpu_util_avg},
                        ))
        
        # Store and notify
        for rec in recommendations:
            self._scaling_recommendations.append(rec)
            logger.info(f"Scaling recommendation: {rec.action.value} {rec.component} - {rec.reason}")
    ...
    def _identify_bottlenecks(self, history: List[ResourceMetrics]) -> List[Dict[str, Any]]:
            ...
            for i in range(len(history[0].gpus)):
                ...
                if gpu_memory and gpu_util:
                    ...
                    if gpu_mem_avg > 80 or gpu_util_avg > 80:
                        bottlenecks.append({
                            "component": f"gpu_{i}",
                            "severity": "high" if max(gpu_mem_avg, gpu_util_avg) > 90 else "medium",
    ...
    def _generate_performance_recommendations(self, history: List[ResourceMetrics]) -> List[str]:
        ...
        if cpu_avg > 80:
            ...
            recommendations.append("Low CPU usage. Consider consolidating workloads or scaling down.")
        ...
        if history[0].gpus:
            gpu_high_usage = any(
                any(m.gpus[i].memory_percent > 80 or m.gpus[i].gpu_percent > 80 for m in history if i < len(m.gpus))
                for i in range(len(history[0].gpus))
            )
            if gpu_high_usage:
                recommendations.append("High GPU usage detected. Consider GPU optimization or model quantization.")
