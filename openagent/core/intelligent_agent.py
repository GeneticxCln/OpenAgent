...
class IntelligentAgent(BaseAgent):
    ...
    def __init__(
        ...
        **kwargs: Any,
        ...
        self.available_models = available_models or ["codellama-7b", "mistral-7b", "ollama:llama3.2"]
        ...
        logger.info(f"Initialized IntelligentAgent '{name}' with {len(self.tools)} tools")
    ...
    async def process_message(self, message: Union[str, BaseMessage]) -> BaseMessage:
        ...
        try:
            ...
            # Model Selection
            model_selection = await self._select_optimal_model(task_type, context, prompt_metadata)
            ...
            response_message = BaseMessage(
                ...
                metadata={
                    ...
                    "context_sections_used": prompt_metadata.get("context_sections_used", []),
    ...
    async def _get_enhanced_context(self, force_refresh: bool = False) -> EnhancedContext:
    ...
    async def _generate_intelligent_response(
        ...
        if self.memory_manager:
            preferred_verbosity = self.memory_manager.get_user_preference("response_verbosity")
        ...
        if prompt_metadata.get("requires_tools", False) and self.tools:
            response = await self._generate_response_with_tools(
                user_prompt, system_prompt, conversation_history, context, task_type, llm
            )
        else:
    ...
    async def _generate_response_with_tools(
        ...
        tools_needed = await self._analyze_tool_requirements(user_prompt, context, task_type)
        ...
        for tool_name, tool_args in tools_needed:
            ...
            if tool:
                try:
                    ...
                    logger.debug(f"Executed tool {tool_name} with success: {result.success}")
                except Exception as e:
    ...
    async def _analyze_tool_requirements(
        ...
        # File operations
        if any(keyword in user_prompt_lower for keyword in ["read file", "show file", "view file", "cat "]):
            # Would need a file reading tool
            pass
        
        # Git operations
        if context.git.is_repo and any(keyword in user_prompt_lower for keyword in ["git status", "git log", "recent changes", "commits"]):
            if any(t.name == "git_tool" for t in self.tools):
                if "status" in user_prompt_lower:
                    ...
                    tools_needed.append(("git_tool", {"subcommand": "log", "args": ["-10"]}))
        
        # System commands
        if task_type in [TaskType.SYSTEM_ADMIN, TaskType.TERMINAL_HELP] and any(
            keyword in user_prompt_lower for keyword in ["run", "execute", "command", "ps", "ls", "find"]
        ):
            if any(t.name == "command_executor" for t in self.tools):
                # Extract potential command from the prompt
                import re
                cmd_match = re.search(r'(?:run|execute)\s+["`]?([^"`\n]+)["`]?', user_prompt_lower)
                if cmd_match:
                    command = cmd_match.group(1)
                    tools_needed.append(("command_executor", {"command": command, "explain_only": True}))
    ...
    async def _record_interaction(
        ...
        if task_type in [TaskType.TERMINAL_HELP, TaskType.SYSTEM_ADMIN, TaskType.DEBUGGING]:
            recent_commands = context.terminal_history[-10:] if context.terminal_history else []
            if recent_commands:
                context_tags = [task_type.value, context.project.project_type.value if context.project.project_type else ""]
                await self.memory_manager.analyze_command_patterns(recent_commands, context_tags)
    ...
    async def _get_personalized_suggestions(
        ...
        recent_commands = context.terminal_history[-5:] if context.terminal_history else []
    ...
    async def refresh_context(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "detection_time_ms": context.performance_metrics.get("detection_time_ms", 0),
        }
