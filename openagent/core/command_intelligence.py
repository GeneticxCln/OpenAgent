...
class CommandCompletionEngine:
    ...
    async def suggest_commands(
        self, partial: str, context: CompletionContext, max_suggestions: int = 10
    ...
    async def complete_arguments(
        self, command: str, partial_arg: str, context: CompletionContext
    ) -> List[CommandSuggestion]:
        """
        ...
        if command.startswith("git"):
            git_suggestions = await self._complete_git_arguments(
                command, partial_arg, context
            )
            suggestions.extend(git_suggestions)

        # Docker-specific completions
        if command.startswith("docker"):
            docker_suggestions = await self._complete_docker_arguments(
                command, partial_arg, context
            )
    ...
    async def _get_history_suggestions(
        ...
        try:
            ...
            prediction_context = PredictionContext(
                ...
                project_type=(
                    context.project_type.value if context.project_type else "unknown"
                ),
