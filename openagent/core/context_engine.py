...
class ContextDetector:
    ...
    def __init__(self, start_path: Path, max_depth: int = 4):
        ...
        self.detection_rules = {
            ProjectType.PYTHON: {
                "files": ["requirements.txt", "setup.py", "pyproject.toml", "manage.py"],
                "extensions": [".py"],
                "frameworks": {
                    "django": ["manage.py"],
    ...
    async def detect_context(self) -> ContextDetectionResult:
        ...
        result.has_docker = any(f.name in ["Dockerfile", "docker-compose.yml"] for f in files)
    ...
    async def _find_project_root(self) -> Optional[Path]:
        ...
        for _ in range(self.max_depth):
            ...
            if any((current_path / f).exists() for f in [".git", "pyproject.toml", "package.json"]):
                return current_path
    ...
    async def _detect_js_frameworks(self, result: ContextDetectionResult, files: List[Path]):
    ...
    async def _detect_python_frameworks(self, result: ContextDetectionResult, files: List[Path]):
