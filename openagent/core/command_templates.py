...
class CommandTemplates:
    ...
    def __init__(self, templates_dir: Optional[Path] = None):
        ...
        self.templates_dir = templates_dir or Path.home() / ".config" / "openagent" / "templates"
    ...
    def _create_builtin_templates(self) -> Dict[str, CommandTemplate]:
        ...
        templates["docker-app-deploy"] = CommandTemplate(
            ...
            project_types=[ProjectType.DOCKER, ProjectType.PYTHON, ProjectType.JAVASCRIPT],
            steps=[
                CommandStep(
    ...
    def list_templates(
        ...
        for template in all_templates.values():
            ...
            if project_type and template.project_types and project_type not in template.project_types:
                continue
    ...
    def suggest_templates(self, context: WorkspaceContext, recent_commands: List[str] = None) -> List[CommandTemplate]:
        ...
        if context.container_info.get("has_dockerfile") or context.container_info.get("has_compose"):
        ...
        if recent_commands:
            for cmd in recent_commands[-5:]:  # Last 5 commands
                if cmd.startswith("git"):
                    ...
                    test_templates = self.list_templates(category=TemplateCategory.TESTING)
    ...
    def execute_template(
        ...
        if template.danger_level >= 3 and not dry_run:
            confirmation = input(f"Template '{template.name}' has danger level {template.danger_level}. Continue? (y/N): ")
    ...
    def _get_default_parameters(self, workspace: Optional[WorkspaceContext]) -> Dict[str, str]:
    ...
    def _check_step_requirements(self, step: CommandStep, workspace: Optional[WorkspaceContext]) -> bool:
    ...
    def _update_template_stats(self, template_name: str, success: bool):
        ...
        if template:
            ...
            if success:
                ...
                new_success_rate = (old_success_rate * (template.usage_count - 1) + 1) / template.usage_count
                template.success_rate = new_success_rate
            else:
                new_success_rate = (template.success_rate * (template.usage_count - 1)) / template.usage_count
                template.success_rate = new_success_rate
            ...
            try:
                ...
                for template_data in data:
                    ...
                    template = CommandTemplate(
                        ...
                        category=TemplateCategory(template_data.get("category", "development")),
                        steps=steps,
                        project_types=[ProjectType(pt) for pt in template_data.get("project_types", [])],
