...
class HuggingFaceLLM:
    ...
    async def load_model(self) -> None:
        ...
        try:
            ...
            try:
                ...
                if self.load_in_8bit or self.load_in_4bit:
                    logger.warning(
                        "bitsandbytes/BnB quantization not available, disabling quantization"
                    )
            ...
            try:
                ...
                arch_name = (
                    getattr(config, "architectures", [None])[0]
                ...
                ) or (isinstance(model_type, str) and "causal" in model_type.lower())
                seq2seq_hint = any(
                    x in str(arch_name).lower()
                    for x in ["t5", "bart", "pegasus", "seq2seq"]
                ) or (isinstance(model_type, str) and "seq2seq" in model_type.lower())
                ...
                if causal_hint and not seq2seq_hint:
                    try:
                        ...
                        logger.info(
                            f"Loaded as seq2seq model via fallback (architecture: {arch_name})"
                        )
                    ...
                    try:
                        ...
                        self.model_type = "seq2seq"
                        logger.info(
                            f"Loaded as seq2seq model (architecture: {arch_name})"
                        )
                    except Exception:
                        ...
                        self.model_type = "causal"
                        logger.info(
                            f"Loaded as causal LM via fallback (architecture: {arch_name})"
                        )
                else:
                    # Unknown: try causal then seq2seq
                    logger.info(
                        f"Unknown architecture {arch_name}, trying causal LM then seq2seq"
                    )
                    try:
                        self.model = AutoModelForCausalLM.from_pretrained(
                            self.model_path, **model_kwargs
                ...
                if (
                    getattr(type(e), "__name__", "") == "OutOfMemoryError"
            ...
            if (
                not model_kwargs.get("device_map")
                and self.device != "cpu"
    ...
    async def generate_response(
        ...
        try:
            ...
            if (
                getattr(type(e), "__name__", "") == "OutOfMemoryError"
                and getattr(self, "device", "cpu") != "cpu"
            ):
            ...
            try:
                _metrics.record_model_inference(
                    self.model_name, success=success, latency=(_t.time() - _start)
                )
            except Exception:
                pass
    ...
    def _format_prompt(
        ...
        if system_prompt is None:
            if any(
                code_model in self.model_path.lower()
                for code_model in ["code", "coder", "star"]
            ):
                system_prompt = (
        ...
        if "llama" in self.model_path.lower():
            ...
            model in self.model_path.lower()
            for model in ["vicuna", "wizard", "openchat"]
        ):
...
"""
        ...
        return await self.generate_response(
            prompt, system_prompt=f"You are an expert {language} developer."
        )
...
"""
    ...
    def _predownload(self) -> None:
        ...
        try:
            ...
            snapshot_download(
                self.model_path, local_files_only=False, allow_patterns=None
            )
