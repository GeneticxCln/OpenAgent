...
class HistoryIntelligence:
        ...
        for block in history_blocks:
            if block.get("input"):
                ...
                contexts.append({
                    ...
                    "project_type": block.get("context", {}).get("project_type", "unknown")
                })
    ...
    async def get_command_suggestions(self, partial: str, context: PredictionContext) -> List[str]:
        """
    ...
    async def _extract_command_sequences(
        ...
        sequences = defaultdict(lambda: {"count": 0, "success": 0, "contexts": set(), "times": []})
        ...
        for seq_len in range(2, self.max_sequence_length + 1):
            for i in range(len(commands) - seq_len + 1):
                ...
                sequences[key]["contexts"].add(contexts[i].get("project_type", "unknown"))
    ...
    async def _extract_corrections(self, history_blocks: List[Dict[str, Any]]) -> Dict[str, str]:
        ...
        for i in range(len(history_blocks) - 1):
            ...
            if (current.get("metadata", {}).get("suggested") and
                next_block.get("input")):
                
                original = current.get("input", "")
                corrected = next_block.get("input", "")
                
                # Simple heuristic: if commands are similar but different
                if (original != corrected and 
                    self._commands_similar(original, corrected)):
    ...
    async def _analyze_user_preferences(
        ...
        avg_command_length = sum(len(cmd.split()) for cmd in commands) / len(commands) if commands else 0
        preferences["complexity_preference"] = "high" if avg_command_length > 4 else "low"
    ...
    def get_command_insights(self, command: str) -> Dict[str, Any]:
        ...
        for pattern in self.patterns_cache.sequences:
            if command in pattern.sequence:
                ...
                insights["success_rate"] = max(insights["success_rate"], pattern.success_rate)
        ...
        insights["typical_next_commands"] = list(set(insights["typical_next_commands"]))[:5]
        insights["common_contexts"] = list(set(insights["common_contexts"]))[:3]
        
        return insights
    
    async def predict_command_outcome(self, command: str, context: PredictionContext) -> Dict[str, Any]:
        """
        ...
        for pattern in patterns.sequences:
            for cmd in pattern.sequence:
                if self._commands_similar(cmd, command):
                    similar_commands.append((cmd, pattern.success_rate, pattern.frequency))
        ...
        weighted_success = sum(
            success * freq for _, success, freq in similar_commands
