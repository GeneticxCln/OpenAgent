...
class EnvironmentDetector:
                    ...
                    for pattern in patterns:
                        if pattern in proc_name or pattern in cmdline:
                            if category == 'databases':
                                ...
                                context.development_servers.append(f"{proc_name} (pid: {proc['pid']})")
                            break
        ...
        try:
            ...
            if result.returncode == 0:
                ...
                cloud_contexts['aws'] = {
                    ...
                    'user': identity.get('Arn', '').split('/')[-1] if identity.get('Arn') else None
                }
        except Exception:
            pass
    ...
    def get_context_summary(self, env_context: EnvironmentContext) -> str:
        ...
        summary.append(f"Resources: CPU {system.cpu_percent:.1f}%, RAM {system.memory_percent:.1f}%, Disk {system.disk_percent:.1f}%")
        
        # Development environment
        dev = env_context.development
        if dev.containers:
            summary.append(f"Containers: {len(dev.containers.get('docker', []))} running")
    ...
    def suggest_optimization_commands(self, env_context: EnvironmentContext) -> List[str]:
        ...
        if system.memory_percent > 80:
            suggestions.extend([
                ...
                "docker system prune" if 'docker' in env_context.development.development_tools else None
            ])
        ...
        if system.disk_percent > 90:
            suggestions.extend([
                ...
                "docker system prune -a" if 'docker' in env_context.development.development_tools else None,
                "npm cache clean --force" if 'npm' in env_context.development.development_tools else None
            ])
