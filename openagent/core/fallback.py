...
class FastFallback:
    ...
    def __init__(self):
        ...
        self.math_patterns = [
            (
                r"(?:what.{0,10}is|calculate|compute|solve)\s*(\d+(?:\.\d+)?)\s*([+\-*/])\s*(\d+(?:\.\d+)?)",
                self._handle_math,
            ),
        ...
        self.common_patterns = [
            (
                r"(?:how to|how do i|help me|show me).{0,30}list.{0,10}files",
                "Use 'ls' to list files, or 'ls -la' for detailed view with hidden files.",
            ),
            (
                r"(?:how to|how do i).{0,30}change.{0,10}directory",
                "Use 'cd directory_name' to change to a directory, or 'cd ..' to go up one level.",
            ),
            (
                r"(?:how to|how do i).{0,30}create.{0,10}(?:folder|directory)",
                "Use 'mkdir folder_name' to create a new directory.",
            ),
            (
                r"(?:how to|how do i).{0,30}delete.{0,10}file",
                "Use 'rm filename' to delete a file. Use 'rm -r dirname' for directories. BE CAREFUL!",
            ),
            (
                r"(?:how to|how do i).{0,30}copy.{0,10}file",
                "Use 'cp source destination' to copy files, or 'cp -r source dest' for directories.",
            ),
            (
                r"(?:check|show|see).{0,20}(?:disk|space|storage)",
                "Use 'df -h' to check disk usage, or 'du -sh *' to see directory sizes.",
            ),
            (
                r"(?:check|show|see).{0,20}(?:processes|running)",
                "Use 'ps aux' to see all processes, or 'top' for real-time process monitor.",
            ),
            (
                r"(?:check|show|see).{0,20}(?:memory|ram)",
                "Use 'free -h' to check memory usage, or 'htop' for detailed system info.",
            ),
            (
                r"(?:download|get|fetch).{0,20}(?:file|url)",
                "Use 'wget URL' or 'curl -O URL' to download files from the internet.",
            ),
            (
                r"(?:what.{0,5}is|explain).{0,10}git",
                "Git is a version control system. Use 'git status' to check repo state, 'git add .' to stage files, 'git commit -m \"message\"' to save changes.",
            ),
            (
                r"(?:what.{0,5}is|explain).{0,10}docker",
                "Docker is a containerization platform. Use 'docker ps' to see running containers, 'docker images' to list images.",
            ),
            (
                r"(?:install|setup).{0,10}python",
                "Install Python: On Ubuntu/Debian: 'sudo apt install python3', On macOS: 'brew install python3', On Windows: download from python.org",
            ),
            (
                r"(?:install|setup).{0,10}node",
                "Install Node.js: Use official installer from nodejs.org, or 'nvm install node' if you have NVM.",
            ),
        ]
