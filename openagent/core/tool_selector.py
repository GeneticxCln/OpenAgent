...
class SmartToolSelector:
        ...
        for name, tool in self.available_tools.items():
            ...
            if "command" in name.lower() or "executor" in name.lower():
                schema["parameters"]["properties"] = {
                    ...
                    "explain_only": {
                        "type": "boolean",
                        "description": "Only explain, don't execute",
                    },
                }
                ...
                schema["parameters"]["properties"] = {
                    "info_type": {
                        "type": "string",
                        "enum": ["overview", "processes", "network", "disk"],
                    }
                }
    ...
    def _extract_search_pattern(self, user_request: str) -> str:
        ...
        identifier_match = re.search(
            r"\b([a-zA-Z_][a-zA-Z0-9_]*(?:[A-Z][a-zA-Z0-9_]*)?)\b", text
        )
        ...
        tech_match = re.search(
            r"\b(\w*\.\w+|\w*Error|\w*Exception|import|class|def|function|method)\b",
            text,
            re.IGNORECASE,
        )
        if tech_match:
...
"""
        ...
        if any(
            k in ur
            for k in ["cpu", "memory", "process", "disk", "system info", "uptime"]
        ):
        ...
        if any(
            k in ur
            for k in ["git ", "commit", "branch", "merge", "status", "diff", "log"]
        ):
        ...
        try:
            if self.llm:
                response = await self.llm.generate_response(
                    prompt, system_prompt="Intent Classifier"
                )
...
"""
            ...
            if self.llm:
                ...
                if callable(generate_json):
                    plan_data = await generate_json(
                        {"prompt": prompt, "schema_hint": "tool_plan_v1"}
                    )
                    # Some implementations may wrap under a key
                    if (
                        isinstance(plan_data, dict)
                        and "plan" in plan_data
                        and not plan_data.get("calls")
                    ):
                        plan_data = plan_data["plan"]
                else:
                    response = await self.llm.generate_response(
                        prompt, system_prompt="Tool Planner"
                    )
                ...
                if isinstance(plan_data, dict):
                    ...
                    return ToolPlan(
                        ...
                        requires_confirmation=plan_data.get(
    ...
    def _create_fallback_plan(self, user_request: str, intent: ToolIntent) -> ToolPlan:
        ...
        if intent == ToolIntent.SYSTEM_INFO and "system_info" in self.available_tools:
            ...
            if any(
                k in t
                for k in ["conflict", "broken", "issue", "problem", "fail", "merge"]
            ):
        ...
        if any(
            k in text
            for k in ["disk usage", "disk space", "df -h", "free space", "check disk"]
        ):
            calls.append(
                ToolCall(
        ...
        if any(
            k in text
            for k in ["list", "ls", "show files", "list files", "directory listing"]
        ):
        ...
        if any(
            k in text
            for k in ["system info", "system information", "overview", "cpu", "memory"]
        ):
            calls.append(
                ToolCall(
                    tool_name="system_info",
        ...
        for call in sorted_calls:
            ...
            try:
                ...
                error_result = ToolResult(
                    success=False, content="", error=f"Tool execution failed: {str(e)}"
                )
