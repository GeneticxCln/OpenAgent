...
class PerformanceManager:
    ...
    async def initialize(self) -> None:
        ...
        try:
            ...
            if self.config.is_resource_monitoring_enabled():
                ...
                self._resource_monitor.add_scaling_callback(self._handle_scaling_recommendation)
    ...
    async def _handle_scaling_recommendation(self, recommendation: ScalingRecommendation) -> None:
        """Handle scaling recommendations."""
        logger.info(f"Scaling recommendation: {recommendation.action.value} {recommendation.component} - {recommendation.reason}")
        
        # Call registered callbacks
        for callback in self._scaling_callbacks:
            try:
                await callback(recommendation) if asyncio.iscoroutinefunction(callback) else callback(recommendation)
            except Exception as e:
    ...
    async def cache_model(self, model_name: str, model: Any, ttl: Optional[int] = None) -> bool:
        """Cache a model."""
    ...
    async def run_performance_optimization(self) -> Dict[str, Any]:
        ...
        logger.info(f"Performance optimization completed: {len(results['optimizations'])} operations")
        return results
...
def get_performance_manager(config: Optional[PerformanceConfigurationManager] = None) -> PerformanceManager:
...
async def initialize_performance_manager(config: Optional[PerformanceConfigurationManager] = None) -> PerformanceManager:
