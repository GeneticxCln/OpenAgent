...
class SmartPromptEngineer:
    ...
    def _initialize_templates(self) -> Dict[TaskType, PromptTemplate]:
        ...
        templates = {
            TaskType.CODE_GENERATION: PromptTemplate(
                ...
                context_sections=["project_context", "git_context", "file_context", "error_context"],
                max_tokens=1024,
                temperature=0.3,
                requires_tools=True,
            ...
            TaskType.CODE_REVIEW: PromptTemplate(
                ...
                context_sections=["project_context", "git_context", "recent_changes", "file_context"],
                max_tokens=1536,
                temperature=0.4,
            ...
            TaskType.DEBUGGING: PromptTemplate(
                ...
                context_sections=["error_context", "recent_commands", "project_context", "git_context"],
                max_tokens=1536,
                temperature=0.2,
                requires_tools=True,
            ...
            TaskType.SYSTEM_ADMIN: PromptTemplate(
                ...
                context_sections=["system_context", "recent_commands", "project_context"],
                max_tokens=1024,
                temperature=0.3,
                requires_tools=True,
    ...
    def _initialize_task_classifiers(self) -> Dict[str, Tuple[TaskType, float]]:
        ...
        return {
            ...
            r"(write|create|generate|implement|build).*?(function|class|script|program|code)": (TaskType.CODE_GENERATION, 0.9),
            r"(create|make|write).*?(app|application|service|module)": (TaskType.CODE_GENERATION, 0.8),
            r"how to.*?(implement|code|program|write)": (TaskType.CODE_GENERATION, 0.7),
            
            # Code review patterns  
            r"(review|check|analyze|examine).*?(code|implementation)": (TaskType.CODE_REVIEW, 0.9),
            r"(what's wrong|problems|issues).*?(code|implementation)": (TaskType.CODE_REVIEW, 0.8),
            r"(improve|optimize|refactor).*?code": (TaskType.CODE_REVIEW, 0.7),
            
            # Debugging patterns
            r"(error|exception|bug|issue|problem|fail|broken)": (TaskType.DEBUGGING, 0.8),
            ...
            r"(explain|what is|how does|why|what's the difference)": (TaskType.EXPLANATION, 0.8),
    ...
    def _initialize_context_prioritizers(self) -> Dict[TaskType, List[str]]:
        ...
        return {
            TaskType.CODE_GENERATION: [
                "project_context", "file_context", "git_context", "framework_context"
            ],
            TaskType.CODE_REVIEW: [
                "file_context", "git_context", "recent_changes", "project_context"
            ],
            TaskType.DEBUGGING: [
                "error_context", "recent_commands", "file_context", "git_context"
            ],
            TaskType.SYSTEM_ADMIN: [
                "system_context", "recent_commands", "environment_context"
            ],
            TaskType.TERMINAL_HELP: [
                "shell_context", "recent_commands", "system_context"
            ],
            TaskType.EXPLANATION: [
                "topic_context", "project_context", "file_context"
            ],
    ...
    async def engineer_prompt(
        ...
        template = self.prompt_templates.get(task_type, self.prompt_templates[TaskType.GENERAL_CHAT])
        
        # Build context sections
        context_data = self._build_context_data(context, template)
        
        # Optimize context window
        context_window = self._optimize_context_window(context_data, template, user_input)
        
        # Generate system prompt
        system_prompt = self._generate_system_prompt(template, context_data, context_window)
    ...
    def select_optimal_model(
        ...
        for model_name in available_models:
            ...
            score = self._calculate_model_score(model_name, config, task_type, context, resource_constraints)
        ...
        if not scored_models:
            ...
            return ModelSelection(
                ...
                fallback_models=available_models[1:5] if len(available_models) > 1 else []
            )
        ...
        return ModelSelection(
            ...
            reasoning=reasoning,
            fallback_models=[model[0] for model in scored_models[1:4]]  # Top 3 alternatives
        )
    ...
    def _calculate_model_score(
        ...
        requires_tools = task_type in [TaskType.CODE_GENERATION, TaskType.DEBUGGING, TaskType.SYSTEM_ADMIN]
        ...
        if context.project.project_type != ProjectType.UNKNOWN:
            ...
            if "code" in strengths and "python" in project_type_lower:
                score += 0.2
            elif "chat" in strengths and project_type_lower in ["generic", "documentation"]:
                score += 0.2
    ...
    def _build_context_data(self, context: EnhancedContext, template: PromptTemplate) -> Dict[str, str]:
        ...
        if "project_context" in template.context_sections:
            project_info = []
            if context.project.project_type != ProjectType.UNKNOWN:
                project_info.append(f"Project Type: {context.project.project_type.value}")
            if context.project.project_name:
                project_info.append(f"Project Name: {context.project.project_name}")
            if context.project.frameworks:
                project_info.append(f"Frameworks: {', '.join(context.project.frameworks)}")
            if context.project.language_map:
                languages = [f"{lang} ({count} files)" for lang, count in context.project.language_map.items()]
                project_info.append(f"Languages: {', '.join(languages)}")
                
            context_data["project_context"] = "Project Context:\n" + "\n".join(f"- {info}" for info in project_info) if project_info else ""
            context_data["project_type"] = context.project.project_type.value
            context_data["project_language"] = max(context.project.language_map.keys(), key=context.project.language_map.get) if context.project.language_map else "Unknown"
        ...
        if "git_context" in template.context_sections and context.git.is_repo:
            ...
            if context.git.modified_files:
                git_info.append(f"Modified files: {', '.join(context.git.modified_files[:5])}")
            if context.git.recent_commits:
                recent = context.git.recent_commits[0]
                git_info.append(f"Last commit: {recent['message'][:50]}...")
                
            context_data["git_context"] = "Git Context:\n" + "\n".join(f"- {info}" for info in git_info) if git_info else ""
        ...
        if "file_context" in template.context_sections:
            ...
            if relevant_files:
                ...
                for file_data in relevant_files:
                    relative_path = str(file_data.path.relative_to(context.project.project_root or Path.cwd()))
        ...
        # Error context
        if "error_context" in template.context_sections and context.errors.recent_errors:
            error_info = []
            ...
            if context.errors.suggested_fixes:
                fixes = [f"{error_type}: {fix}" for error_type, fix in list(context.errors.suggested_fixes.items())[:2]]
                error_info.extend(fixes)
                
            context_data["error_context"] = "Recent Errors:\n" + "\n".join(f"- {info}" for info in error_info)
        ...
        if "system_context" in template.context_sections:
            ...
            if "SHELL" in context.environment_vars:
                sys_info.append(f"Shell: {Path(context.environment_vars['SHELL']).name}")
            ...
            if "PWD" in context.environment_vars:
                sys_info.append(f"Working Directory: {Path(context.environment_vars['PWD']).name}")
                
            context_data["system_context"] = "System Context:\n" + "\n".join(f"- {info}" for info in sys_info) if sys_info else ""
            context_data["shell_type"] = Path(context.environment_vars.get("SHELL", "bash")).name
        ...
        if "framework_specific_guidance" in template.context_sections:
            ...
            for framework in context.project.frameworks:
                if framework == "fastapi":
                    guidance.append("- Use FastAPI best practices: dependency injection, proper HTTP status codes")
                elif framework == "django":
                    guidance.append("- Follow Django conventions: models, views, templates pattern")
        ...
        if "minimal_context" in template.context_sections:
            context_data["minimal_context"] = f"Working in: {context.project.project_name or 'current directory'}"
        
        return context_data

    def _optimize_context_window(self, context_data: Dict[str, str], template: PromptTemplate, user_input: str) -> ContextWindow:
            ...
            if section in context_data:
                ...
                if used_tokens + section_tokens <= context_window.available_tokens:
                    ...
                    if remaining_tokens > 50:  # Minimum useful size
                        truncated_content = " ".join(context_data[section].split()[:remaining_tokens])
    ...
    def _generate_system_prompt(self, template: PromptTemplate, context_data: Dict[str, str], context_window: ContextWindow) -> str:
        ...
        filtered_context = {
            key: value for key, value in context_data.items()
            if key in context_window.prioritized_sections or key in ["project_type", "project_language", "shell_type", "operating_system"]
        }
    ...
    def _generate_model_selection_reasoning(
        ...
        if config.get("supports_tools", False) and task_type in [TaskType.CODE_GENERATION, TaskType.DEBUGGING]:
