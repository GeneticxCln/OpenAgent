...
class EnhancedContextDetector(ContextDetector):
    ...
    async def detect_enhanced_context(self) -> EnhancedContext:
        ...
        try:
            ...
            if context.project.project_root:
                context.git = await self._detect_git_context(context.project.project_root)
    ...
    async def _detect_git_context(self, project_root: Path) -> GitContext:
        ...
        try:
            ...
            # Get current branch
            branch_result = await self._run_git_command("rev-parse --abbrev-ref HEAD", project_root)
            if branch_result["success"]:
                git_context.branch = branch_result["output"].strip()
            
            # Get remote URL
            remote_result = await self._run_git_command("config --get remote.origin.url", project_root)
            ...
            status_result = await self._run_git_command("status --porcelain", project_root)
            ...
            # Get status summary
            status_summary_result = await self._run_git_command("status --short", project_root)
            ...
            count_result = await self._run_git_command("rev-list --count HEAD", project_root)
    ...
    async def _run_git_command(self, command: str, cwd: Path, timeout: float = 10.0) -> Dict[str, Any]:
    ...
    async def _calculate_file_relevance_score(
        ...
        relative_path = str(file_path.relative_to(project_context.project_root or Path.cwd()))
        ...
        config_patterns = [
            ...
            r'.*\.env.*$', r'Dockerfile.*', r'.*\.conf$', r'Makefile.*', 
            r'.*requirements.*\.txt$', r'package\.json$', r'tsconfig\.json$'
        ]
        ...
        # Documentation files
        if any(ext in file_path.suffix.lower() for ext in ['.md', '.rst', '.txt']) and \
           any(name in file_path.name.lower() for name in ['readme', 'changelog', 'todo', 'notes']):
            score += 0.4
    ...
    async def _extract_error_context(self) -> ErrorContext:
        ...
        try:
            ...
            error_context.suggested_fixes = self._generate_error_fixes(error_context.error_patterns)
    ...
    def _get_relevant_env_vars(self) -> Dict[str, str]:
        ...
        important_vars = [
            'PATH', 'SHELL', 'TERM', 'USER', 'HOME', 'PWD',
            'PYTHON_PATH', 'PYTHONPATH', 'VIRTUAL_ENV', 'CONDA_DEFAULT_ENV',
            'NODE_ENV', 'NPM_CONFIG_PREFIX', 'JAVA_HOME',
            'GIT_AUTHOR_NAME', 'GIT_AUTHOR_EMAIL',
            'EDITOR', 'VISUAL', 'PAGER',
            'LANG', 'LC_ALL', 'LANGUAGE',
            'OPENAGENT_MODEL', 'OPENAGENT_EXPLAIN',
