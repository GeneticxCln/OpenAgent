...
class MemoryManager:
    ...
    async def get_or_create_session(
        ...
        session = ConversationSession(
            ...
            working_directory=str(context.project.project_root) if context and context.project.project_root else None,
        )
    ...
    async def learn_user_preference(
        ...
        if preference_type in self.user_preferences:
            ...
            if isinstance(preference_value, (int, float)) and isinstance(pref.preference_value, (int, float)):
                weight = pref.confidence
                pref.preference_value = (pref.preference_value * weight + preference_value) / (weight + 1)
            else:
    ...
    async def analyze_command_patterns(self, commands: List[str], context_tags: List[str]) -> List[CommandPattern]:
        ...
        for seq_length in range(2, min(6, len(commands) + 1)):
            for i in range(len(commands) - seq_length + 1):
                ...
                if pattern_id in self.command_patterns:
                    ...
                    pattern.context_tags = list(set(pattern.context_tags + context_tags))
                else:
                    pattern = CommandPattern(
    ...
    def get_relevant_patterns(
        ...
        relevant_patterns.sort(
            key=lambda p: (p.frequency, p.last_seen), 
            reverse=True
        )
    ...
    async def generate_personalized_suggestions(
        ...
        for pattern in relevant_patterns[:3]:  # Top 3 patterns
            if len(recent_commands) >= 2:
                ...
                if recent_sequence == pattern.command_sequence[:2] and len(pattern.command_sequence) > 2:
        ...
        if preferred_verbosity == "concise" and current_task_type in [TaskType.TERMINAL_HELP, TaskType.SYSTEM_ADMIN]:
            suggestion = PersonalizedSuggestion(
        ...
        if context.errors.error_patterns:
            common_errors = sorted(context.errors.error_patterns.items(), key=lambda x: x[1], reverse=True)
            if common_errors:
        ...
        if context.git.is_repo and context.git.modified_files and len(context.git.modified_files) > 5:
            suggestion = PersonalizedSuggestion(
...
class PersonalizedSuggestionEngine:
    ...
    async def get_suggestions(
        ...
        if not force_refresh and cache_key in self.suggestion_cache:
            ...
            if cached_suggestions and time.time() - cached_suggestions[0].times_shown < self.cache_ttl:
    ...
    async def record_suggestion_interaction(
        ...
        if suggestion:
            ...
            if feedback:
                await self._learn_from_suggestion_feedback(suggestion, feedback, accepted)
    ...
    async def _learn_from_suggestion_feedback(
        ...
        if "different model" in feedback_lower and suggestion.suggestion_type == "model_optimization":
