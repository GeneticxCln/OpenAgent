...
class WebSocketHandler:
    ...
    def __init__(
        ...
        *,
        ...
        authenticate_token: Optional[Callable[[str], Awaitable[Optional[Dict[str, Any]]]]] = None,
    ) -> None:
        self._send = send
        self._lookup_agent = lookup_agent
        self._authenticate_token = authenticate_token

    async def handle(self, connection_id: str, info: ConnectionInfo, raw_text: str) -> None:
        """Handle a single incoming message from a client."""
        try:
            message = WebSocketMessage.from_json(raw_text)
        except Exception:
            await self._send(connection_id, WebSocketMessage(type=MessageType.ERROR, data={
                "error": "INVALID_JSON",
        ...
        try:
            if message.type == MessageType.PING:
                await self._send(connection_id, WebSocketMessage.create_response(message, MessageType.PONG, {}))
                return
            ...
            if message.type == MessageType.CONNECT:
                ...
                await self._send(connection_id, WebSocketMessage.create_response(message, MessageType.CONNECT, {
            ...
            if message.type == MessageType.CHAT:
                ...
                if not agent:
                    await self._send(connection_id, WebSocketMessage.create_error(message, f"Agent '{agent_name}' not found", "AGENT_NOT_FOUND"))
                    return
                ...
                if not stream:
                    ...
                    await self._send(connection_id, WebSocketMessage.create_response(message, MessageType.CHAT_RESPONSE, {
                        "message": reply if isinstance(reply, str) else getattr(reply, "content", str(reply)),
                        "agent": agent_name,
                    }))
                else:
                    ...
                    await self._send(connection_id, WebSocketMessage(type=MessageType.CHAT_STREAM, data={"content": ""}, stream_id=stream_id, is_stream_start=True))
                    buf = []
                    for i, token in enumerate(chunks, 1):
                        ...
                        if len(buf) >= 20 or i == len(chunks):
                            part = " ".join(buf)
                            buf = []
                            await self._send(connection_id, WebSocketMessage(type=MessageType.CHAT_STREAM, data={"content": part}, stream_id=stream_id))
                            await asyncio.sleep(0.01)
                    await self._send(connection_id, WebSocketMessage(type=MessageType.CHAT_END, data={}, stream_id=stream_id, is_stream_end=True))
                return

            # Unknown type
            await self._send(connection_id, WebSocketMessage.create_error(message, f"Unsupported message type: {message.type}", "UNSUPPORTED_TYPE"))
        except Exception as e:
            await self._send(connection_id, WebSocketMessage.create_error(message, str(e), "HANDLER_ERROR"))
